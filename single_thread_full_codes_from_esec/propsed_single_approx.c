...
int main()
{
    ...
    for (int q= 0 ; q< num_temp_tets;q++)
    {
        ...
        exact_knn_projected_(&result_test,&reference,query.row_data[query_order[q]],query_projected[q], nearest_index, test_k, q,num_ref_points, bucket_size);
        ground_truth = KNN_one_row (&reference, &query, test_k,Euclidean, q);
        cout<<"-------------"<<endl;
        print_vector_int(result_test[q]);
        cout<<"must be equal to:"<<endl;
        print_vector_int(ground_truth);
        cout<<"-------------"<<endl;
        cout<<endl;
        matches = 0;
