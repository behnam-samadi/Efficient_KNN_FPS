    ...
    node * Create_KD_Tree(vector<vector<double>>* all_points)
{
    ...
    cout<<temp_is_set.size()<<" "<<temp_is_set[0].size()<<" "<<temp_limits.size()<<" "<<temp_limits[0].size()<<endl;
    cout<<"ina bood:"<<endl;
    boundries.is_set = temp_is_set;
    boundries.limits = temp_limits;
    //cout<<boundries.is_set.size()<<" "<<boundries.is_set[0].size()<<" "<<boundries.limits.size()<<" "<<boundries.limits[0].size()<<endl;
    //cout<<"ina bood:"<<endl;
...
void KNN_Exact_rec(vector<double> query, int k, priority_queue<pair<double, int>>* knn, int root_index, double current_max_dist, vector<vector<double>>* all_points)
{
    ...
    //first examine the leaf node
    max_dist = examine_point(knn, k, this->tree[leaf].point, query,this->tree[leaf].point_index);
    bool from_left = ((leaf%2) == 1);
    int current_node = (leaf-1)/2;
    bool finish_upward_traverse = false;
    while(!finish_upward_traverse)
    {
        ...
        if (from_left)
        {
            ...
            {
                //cout<<endl<<"***"<<"back_track:"<<"***"<<endl;
                KNN_Exact_rec(query, k, knn, 2*current_node+2 ,max_dist, all_points);
            }
        }
        }
        else
            ...
            if (this->tree[2*current_node+1].is_set)
        {
            ...
            {
                //cout<<endl<<"***"<<"back_track:"<<"***"<<endl;
                KNN_Exact_rec(query, k, knn, 2*current_node+1, max_dist, all_points);
            }
        }
        }
    if (current_node<=root_index) finish_upward_traverse = true;
    else
    {
    if ((*knn).size() == k)
    {
        if (ball_within_bound((*all_points)[(*knn).top().second] , (*knn).top().first, this->tree[current_node].boundries))
        {
            //bool ball_within_bound(vector<double> farthest_point, double max_dist, node_boundries boundries)
            finish_upward_traverse = true;
            vector<double> farthest_point = (*all_points)[(*knn).top().second];
            cout<<"---------------------"<<endl<<" search finished for point["<<farthest_point[0]<<","<<farthest_point[1]<<","<<farthest_point[2]<<"] and max dist = "<<max_dist<<" and boundries: ";
            print_vector_2D(this->tree[current_node].boundries.limits);
            cout<<endl<<"current_node" <<current_node<<endl;
            cout<<"boolean boundries:"<<endl;
            print_vector_2D_bool(this->tree[current_node].boundries.is_set);
            cout<<"---------------------"<<endl<<endl;
        }
    }
}
...
void KNN_Exact(vector<double> query, int k, int*result, vector<vector<double>>* all_points)
{
    //int root_index = 0;
    priority_queue<pair<double, int>> result_queue;
    KNN_Exact_rec(query, k , &result_queue, 0,0, all_points);
    int index = 0;
...
bool point_in_boundries(vector<double> point, node_boundries boundries)
{
    for (int i = 0 ; i < point.size() ; i++)
    {
        for (int j= 0 ; j < 2; j++)
        {
            if (boundries.is_set[i][j])
            {
                if (j ==0)
                {
                    if (point[i] < boundries.limits[i][i]) return false;
                }
                else if(j==1)
                {
                    if (point[i] > boundries.limits[i][i]) return false;
...
{
    bool result = true;
    if (!point_in_boundries(farthest_point, boundries))
    {
        return false;
    }
    for (int dim= 0 ; dim <farthest_point.size(); dim++)
    {
        for (int kind =0 ; kind<2;kind++)
        {
            if (boundries.is_set[dim][kind]== true) 
            {
                if (abs(boundries.limits[dim][kind] - farthest_point[dim])<= max_dist)
                {
                    result = false;
                    break;
                }
...
int main()
{
    ...
    Frame query("reformed_dataset/1_gr.bin", 640);
    ...
    for (int q= 0 ; q< num_temp_tets;q++)
    {
        ...
        num_exam = 0;
        reference_tree.KNN_Exact(query.data[q],test_k, result_temp[q], &(reference.data));       
    ...
    cout<<endl<<"num_exam_sum/num_temp_tets "<< num_exam_sum/num_temp_tets;
    //exit(0);
    
    int score;
    for (int i = 0 ; i < num_temp_tets; i++)
    {
        correct_answer =  KNN_one_row (&reference, &query, test_k,Euclidean, i);
        //print_vector_int(correct_answer);
        //cout<<endl<<"and returned answer:"<<endl;
        //for (int p = 0 ; p < test_k; p++)
        //{
        //    cout<<result_temp[i][p]<<endl;
        //}
        //cout<<"-------------------------------------------------------------------"<<endl;
        int matches = 0;
        for (int j = 0 ;j < test_k; j++)
        {
            for (int c = 0 ; c< test_k;c++)
            {
                if (result_temp[i][j] == correct_answer[c])
                {
                    matches++;
                    break;
                }
        ...
        if (matches == test_k) score++;
        else cout<<endl<<"fault in: "<<i<<endl;
    }
    cout<<endl<<score;
