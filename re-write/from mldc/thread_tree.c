...
void * root_func (void* void_data)
{
    ...
    while(current_query_index <= max_job_index)
    {
        ...
        if (current_query < middle_value)
        {
            ...
            //cout<<"wrote to left: "<<current_query_index<<" in root"<<endl;
...
void * inner_node_func (void* void_data)
{
    ...
    while(cont)
    {
        ...
        while (read_stream->size() ==0){//cout<<"thread "<<id_for_check<<"is waiting";
    }
        ...
        if (query_index!= -1)
        {
            cout<<"thread "<<id_for_check<<" got the point "<<query_index<<endl;
...
void * leaf_node_func (void* void_data)
{
    ...
    while(cont)
    {
        ...
        {                
            cout<<"thread "<<id_for_check<<" got the point "<<temp_pair.index<<endl;
...
/*for (int i = 0; i <=max_job_index;i++ )
    ...
    //exit(0);
    cout<<"start running the job!"<<endl;
    start = true;
