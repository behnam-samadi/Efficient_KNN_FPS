...
int main()
{
    ...
    //Frame reference("reformed_dataset/0_cut_10000.txt");
    ...
    cout<<reference.data.size()<<endl<<query.data.size()<<endl;
    ...
    for (int q= 0 ; q< num_temp_tets;q++)
    {
        ...
        cout<<num_exam<<endl;
        runTime2 =-omp_get_wtime();
        correct_result = KNN_one_row(&reference, &query, test_k , Euclidean, q);
        runTime2 += omp_get_wtime();
        time_exact_sum+=runTime2;

        //vector<int> KNN_one_row (Frame * reference, Frame * query, int K,dist_metric metric, int index){
        //void KNN_Exact(vector<float> query, int k, int*result)
    }
    cout<<"average number of Euclidean distances calculations:"<<(num_exam_sum / num_temp_tets)<<endl;
    cout<<"average rate of cutoff: "<<((float)(num_exam_sum / num_temp_tets)/(float)reference.data.size())<<endl;
    cout<<endl<<time_sum / num_temp_tets<<endl;
    cout<<"speed up:"<<time_exact_sum/time_sum;
    exit(0);
