...
#include <omp.h>
#include <time.h>
#define fix_reference_size 18334
#define fix_query_size 120000
...
{   
    ...
    Frame reference = read_data("0000000000.bin", 4, frame_channels, fix_reference_size);
    Frame query = read_data("0000000001.bin", 4, frame_channels, fix_query_size);
    int num_ref_points = reference.data.size();
    num_ref_points = fix_reference_size;
    int num_query_points = query.data.size();
    ...
    cout<<endl<<"test done with score "<<score<<" "<<num_calc_dis_test/num_temp_tets<<" "<<avg_test_time<<" "<<avg_ex_time<<" "<<avg_ex_time/avg_test_time <<endl;
    exit(0);
